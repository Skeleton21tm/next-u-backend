    ...
    fastify.route({
        ...
        handler: async (req, reply) => {
            try {
                ...
                const user = await database.query('SELECT email, username, token FROM user WHERE id = $1', [userId]);
                if (user.rows.length === 0) {
                    reply.code(404).send({ message: 'User not found' });
                } else {
                    reply.send({ user: user.rows[0] });
                }
            } catch (error) {
                log.error(error);
        ...
        handler: async (req, reply) => {
            try {
                ...
                const result = await database.query(
                    'INSERT INTO user (username, email, password, token) VALUES ($1, $2, $3, $4) RETURNING username, email, token',
